...
### צעד 4: שמירה רציפה מרגע זה
```
┌──────────────────────────────────────────────────────────┐
│  ⚠️ מרגע שהאתחול הושלם — שמירה רציפה פעילה!            │
│                                                          │
│  אחרי כל תשובה משמעותית של המשתמש:                      │
│  1. עדכן checkpoint.json (agent, question, notes)        │
│  2. עדכן epic file (אם יש תוכן חדש)                    │
│  3. ב-50% Context → 🛑 עצור + שמור + הצע compact        │
└──────────────────────────────────────────────────────────┘
```
...
### כלל 3: שמירה רציפה
שמור ל-`checkpoint.json` אחרי **כל תשובה משמעותית**, לא רק בסוף.
עדכן: `current_epic`, `current_agent`, `question_number`, `notes`.
